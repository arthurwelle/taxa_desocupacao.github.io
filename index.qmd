...
Última atualização em: `r Sys.time()`.
...
#selecionando dados e dando novos nomes as colunas
d1 <- d %>%  dplyr::mutate(date = `Trimestre (Código)`,
                           date = lubridate::yq(paste0(date)),
                           Valor = Valor / 100) %>% 
            dplyr::select(date, Valor, Sexo)
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Taxa de desocupação por sexo - Brasil (2012-2023)",
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Nível de desocupação - Brasil (2012-2023)",
...
d1 <- d %>%  dplyr::mutate(date = `Trimestre (Código)`,
            ...
            dplyr::select(date, Valor, FaixaEtaria)
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Taxa de desocupação por faixa etária - Brasil (2012-2023)",
...
d1 <- d %>% 
            dplyr::mutate(date = `Trimestre Móvel (Código)`,
            ...
            dplyr::select(date, Grupo, Valor, CodGrupo ) %>%
            dplyr::mutate(date = lubridate::ymd(paste0(date, "01"))) %>% 
            dplyr::arrange(CodGrupo,date) %>%
            dplyr::group_by(CodGrupo) %>%
            dplyr::mutate(MM12m = MM12m(Valor)) %>% 
            dplyr::group_by(date) %>%
            dplyr::mutate(STotal = sum(MM12m[which(Grupo == "Total")])) %>%
            dplyr::group_by(CodGrupo) %>%
            dplyr::mutate(diff = ((MM12m - lag(MM12m, n = 1L))/lag(STotal, n = 1L)) ,
                          diff_anual = ((Valor - lag(Valor, n = 12L))/lag(STotal, n = 1L)) ) %>% 
            dplyr::mutate(Grupo = recode(Grupo,
...
# seleciona casos
d2 <- d1 %>%  dplyr::filter(CodGrupo==31722 | # empregodo privado com carteira
...
g <- ggplot2::ggplot()  +
          ...
          scale_x_date(expand = c(0, 0), 
                       ...
                       date_labels = "%Y") +
          scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
          labs(title = "Contribuição para a variação da média móvel 12 meses do total de ocupados por posição na ocupação e formalização (2017-2023)",
               x = "", 
...
d1 <- d %>% 
            ...
            dplyr::arrange(CodGrupo,date) %>%
            dplyr::group_by(date) %>%
            dplyr::mutate(STotal = sum(Valor[which(Grupo == "Total")])) %>%
            dplyr::group_by(CodGrupo) %>%
            dplyr::mutate(diff = ((Valor - lag(Valor, n = 1L))/lag(STotal, n = 1L)) ,
...
g <- ggplot2::ggplot()  +
          ...
          scale_x_date(expand = c(0, 0), 
                       ...
                       date_labels = "%Y") +
          scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
          labs(title = "Contribuição para a variação do total de ocupados por posição na ocupação e formalização (2017-2023)",
               x = element_blank(), 
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "Milhões de pessoas",
       title = "Quantidade de pessoas subocupadas - Brasil (2012-2023)",
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "Milhões de pessoas",
       title = "Quantidade de pessoas desalentadas - Brasil (2012-2023)",
...
# 12) Força de trabalho potencial
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "Milhões de pessoas",
       title = "Quantidade de pessoas na força de trabalho potencial - Brasil (2012-2023)",
...
# 13) Quantidade de pessoas subutilizadas
...
# 14) Composição da subutilização da força de trabalho no Brasil
...
# selecionando dados e dando novos nomes as colunas
d1 <- d1 %>%  dplyr::mutate(Desocupados = Valor/1000, date = `Trimestre Móvel (Código)`) %>%
              dplyr::select(date, Desocupados)
d2 <- d2 %>%  dplyr::mutate(Subocupados = Valor/1000, date = `Trimestre Móvel (Código)`) %>%
              dplyr::select(date, Subocupados)
d3 <- d3 %>%  dplyr::mutate(Desalentados = Valor/1000, date = `Trimestre Móvel (Código)`) %>%
              dplyr::select(date, Desalentados)
d4 <- d4 %>%  dplyr::mutate(FTPotencial = Valor/1000, date = `Trimestre Móvel (Código)`) %>%
              dplyr::select(date, FTPotencial)
...
d <- dplyr::left_join(d, d3, by="date")
d <- dplyr::left_join(d, d4, by="date")
...
# cria gráfico ::ggplot
g <- ggplot2::ggplot(d)  +
          geom_line(aes(x=date, y=Desocupados, color = "Desocupados"), size = 1.5) +
          geom_line(aes(x=date, y=Subocupados, color = "Subocupados"), size = 1.5) +
          geom_line(aes(x=date, y=Desalentados, color = "Desalentados"), size = 1.5) +
          geom_line(aes(x=date, y=ProcurouNaoAssumiria, color = "Força de Trabalho potencial (exceto desalento)"), size = 1.5) +
          labs(x = "", y = "Milhões de pessoas",
...
# 15) Rendimento efetivo médio todos os trabalhos
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Rendimento médio efetivo de todos os trabalhos - Brasil (2012-2023)",
...
# 16) Rendimento efetivo médio trabalhos principal, por posição na ocupação
...
```{r}
g <- ggplot()  +
  geom_col(data = d2[d2$variavel != "Total",],
  ...
  labs(x = "",
       y = "",
       title = "Rendimento médio trabalho principal, habitualmente recebido - Brasil (2023)",
       subtitle = "Pessoas de 14 anos ou mais de idade, ocupadas na semana de referência, com rendimento de trabalho",
       caption = "Fonte: PNADC-Mensal") + 
  theme_bw() +
  tema_padrao +
...
# 17) Massa real de rendimentos de todos os trabalhos
...
```{r}
g <- ggplot()  +
  geom_line(data = d1[d1$Grupo == "Habitual",],
            aes(x = date, 
                y = Valor),
            size = 1.5) +  
  scale_x_date(date_breaks = "1 year", 
  ...
  labs(x = "",
       y = "Bilhões R$",
       title = "Massa real de rendimentos - Brasil (2012-2023)",
       subtitle = "Rendimento efetivo total de todos os trabalhos",
       caption = "Fonte: PNADC-Mensal") + 
  theme_bw() +
  tema_padrao 
...
```{r}
g <- ggplot()  +
  geom_line(data = d1[d1$Grupo == "Efetiva",],
            aes(x = date, 
                y = Valor),
            size = 1.5) +  
  scale_x_date(date_breaks = "1 year", 
...
f_ggsave(g, "PNADCT_MassaRendaEfeTodosTrab")
g
```



# 18) Percentual de pessoas contribuintes de instituto de previdência em qualquer trabalho entre ocupados
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Taxa de cobertura previdênciária - Brasil (2012-2023)",
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Salário mínimo real (1940-2023)",
...
# 27) Horas médias trabalhadas por raça
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Horas médias habitualmente trabalhadas no trabalho principal por cor ou raça - Brasil (2012-2023)",
       caption = "Fonte: PNADC-Trimestral") + 
  theme_bw() +
  tema_padrao 
...
# 28) Rendimento real médio por sexo
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Renda média habitual de todos os trabalhos por Sexo - Brasil (2012-2023)",
       caption = "Fonte: PNADC-Trimestral") + 
  theme_bw() +
  tema_padrao 
...
# 29) População ocupada por contribuição ao instituto de previdência
...
d1 <- d %>%  dplyr::mutate(date = `Trimestre (Código)`,
                           ...
                           date = lubridate::yq(paste0(date)))%>% 
            dplyr::select(date, Valor, contribui)
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "milhões",
       title = "População ocupada por contribuição ao instituto de previdência (2012-2023)",
       caption = "Fonte: PNADC-Trimestral") + 
  theme_bw() +
  tema_padrao 
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Taxa de subocupação por insuficiência de horas trabalhadas(2012-2023)",
       caption = "Fonte: PNADC-Trimestral") + 
  theme_bw() +
  tema_padrao 
...
# 31) Rendimento real médio efetivo por posição de ocupação
...
d1 <- d %>%  dplyr::mutate(date = `Trimestre (Código)`,
            ...
            dplyr::select(date, Valor, pos_ocup)
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "",
       title = "Rendimento Real médio habitual do trabalho principal por posição na \nocupação (2012-2023)",
       caption = "Fonte: PNADC-Trimestral") + 
  theme_bw() +
  tema_padrao 
...
# 32) população ocupada por setor

```{r}
d <- sidrar::get_sidra(api = "/t/5434/n1/all/v/4090/p/all/c693/allxt") #População empregada nos diferentes setores 

d1 <- d %>%  dplyr::mutate(date = `Trimestre (Código)`,
                           date = lubridate::yq(paste0(date)),
                           setor = `Grupamento de atividades no trabalho principal - PNADC`,
                           codigo = `Grupamento de atividades no trabalho principal - PNADC (Código)`)
d1 <- filter(d1, date== "2022-04-01")
d1 <- mutate(d1, setor = ifelse(codigo== 33357, "Agropecuária e correlatos", setor))
d1 <- mutate(d1, setor = ifelse(codigo== 39325, "Serviço Público", setor))
d1 <- mutate(d1, setor = ifelse(codigo== 33361, "Comércio", setor))
d1 <- mutate(d1, setor = ifelse(codigo== 33364, "Administrativo/Comercial", setor))
d1 <- mutate(d1, setor = ifelse(codigo== 33362, "Transporte e correlatos", setor))


d1 <- select(d1, date, Valor, setor)

g <- ggplot()  +
  geom_col(data = d1,
            aes(x = reorder(setor, Valor), 
                y = Valor),
            fill = "steelblue",
            color = "black",
            size = 0.7) +
  ...
  labs(x = "",
       y = "milhões de pessoas",
       title = "População ocupada nos diferentes setores - Brasil (2022)",
       caption = "Fonte: PNADC-Trimestral") + 
  theme_bw() +
  tema_padrao

f_ggsave(g, "PNADCT_Pessoas_por_setor")

g
```

# 33) População ocupada por tamanho do estabelecimento

```{r}

d2 <- sidrar::get_sidra(api = "/t/6413/n1/all/v/10441/p/all/c2/6794/c790/allxt") #População ocupada dividida pelo tamanho do empreendimento que trabalha (trabalho principal)

d2 <- d2 %>%  
  dplyr::mutate(date = Ano) %>%
  dplyr::mutate(tamanho = `Tamanho do empreendimento` ) %>%
  dplyr::mutate( tamanho_codigo = `Tamanho do empreendimento (Código)`) %>%
...
g <- ggplot()  +
  ...
  labs(x = "",
       y = "milhões",
       title = "População ocupada por tamanho de empreendimento (2012-2019)",
